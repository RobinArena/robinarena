...
const {
  account, authenticated, busy: walletBusy, error: walletError, wallet,
...
function applyAccountSettings(): void {
  ...
  strategy.value = account.value.settings.strategy;
}
...
onMounted(async () => {
  ...
  if (authenticated.value) {
    try {
      const existing = await getExistingAccount();
      if (existing) { applyAccountSettings(); await loadPrivateData(); }
  ...
  <div class="page-shell userapp-page">
    ...
    <section v-if="!authenticated" class="userapp-entry">
      <div>
        ...
        <p>You will sign a login message. Creating the agent wallet requires a second signature that derives its trading key.</p>
      </div>
    ...
    <section v-else-if="!account" class="userapp-entry">
      <div>
        <h2>Create your agent wallet</h2>
        <p>The derived wallet is dedicated to Robinhood Chain trading. RobinArena stores its encrypted key so the selected model can execute your strategy.</p>
      </div>
      <button class="button button-primary" type="button" :disabled="walletBusy" @click="createAgentWallet">
        {{ walletBusy ? "Waiting for signature" : "Create agent wallet" }}
      </button>
    </section>
      ...
      <div class="userapp-workspace">
        <main class="userapp-main">
          <section class="userapp-section">
            ...
            <UserModelPicker :models="modelData?.models || []" :selected-id="selectedModel" :disabled="isActive" @select="selectedModel = $event" />
            ...
            <button class="button button-primary" type="button" :disabled="saving || isActive || !selectedModel" @click="saveSettings">{{ saving ? "Saving" : "Save model and strategy" }}</button>
          </section>
        ...
        <aside class="userapp-sidebar">
          ...
          <section class="control-panel">
            ...
            <p v-else>Save a strategy and deposit ETH before starting live execution.</p>
            <button v-if="!isActive" class="button button-primary control-button" type="button" :disabled="loading || !hasFunds" @click="setStatus('active')"><Icon name="ph:play-fill" />Start trading</button>
