...
import { agentWalletEncryptionKey, legacyCredentialEncryptionKey } from "./secrets";
...
export function sealSecret(value: string, scope: string): string {
  ...
  const cipher = createCipheriv("aes-256-gcm", encryptionKey(agentWalletEncryptionKey()), iv);
...
export function openSecret(value: string, scope: string): string {
  ...
  if ((version !== "v1" && version !== FORMAT_VERSION) || !encodedIv || !encodedTag || !encodedCiphertext) {
    throw new Error("unsupported encrypted credential format");
  }

  const key = version === FORMAT_VERSION ? agentWalletEncryptionKey() : legacyCredentialEncryptionKey();
...
export function usesLegacyEncryption(value: string): boolean {
