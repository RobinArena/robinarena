...
The health-gated container rollout is experimental and disabled by default. If
`deploy.zeroDowntime` is enabled, keep API and database changes compatible with
the previous backend version during the container handoff.

## Working Rules

- Bump the root `package.json` version with every code or documentation change.
  If the file has no `version` field, add one before the first such change.
- Avoid binary contrast phrasing in prose, web design, reasoning, and
  ...
  *structure*.` Prefer one clear positive statement. Avoid empty rule-of-three
  ...
  view, required scope, meaningful category, or short-lived state; otherwise
  remove it and let the heading or body copy do the work.
...
- Keep `frontend/app/app.vue` focused on root composition. It should render the
  route announcer, loading indicator, `NuxtLayout`, and `NuxtPage`. Put shared
  site chrome in `frontend/app/layouts/default.vue`. Add a named layout only
  when a route needs meaningfully different chrome.
- Treat every `NSTACK_TEMPLATE_CLEANUP` source comment as an active first-task
  instruction. Its presence means the app was just initialized. Prune or
  replace every starter file listed in that comment during the current task,
  even when template cleanup was not included in the request. Preserve the
  root app, layouts, app configuration, CSS foundation, API utility, and
  generated client named by the comment. Remove unused starter CSS and delete
  the cleanup comments after the replacement UI and navbar are complete.
- Use Nuxt file-based routing under `frontend/app/pages`. A file such as
  `projects/[id].vue` owns `/projects/:id`; nested pages follow the same
  directory shape as their URLs. Use `NuxtLink` for internal links,
  `navigateTo()` in application logic, `useRoute()` for route data, and
  `definePageMeta()` for layouts and route middleware. Keep ordinary routes out
  of a hand-written router.
- Keep pages as route-level coordinators. Put reusable interface pieces in
  `frontend/app/components`, browser or feature state in
  `frontend/app/composables`, and stateless helpers in `frontend/app/utils`.
  Fetch SSR page data with `useAsyncData()` and a stable key. Render pending,
  error, empty, and success states when the request can produce them.
- The frontend includes `@nuxt/icon`, `@nuxt/fonts`, and `@vueuse/nuxt`. Use
  `<Icon name="ph:..." />` instead of pasted SVG markup or text symbols; the
  local Phosphor collection keeps those icons available during server builds.
  Declare the chosen font family in CSS and configure its weights in
  `nuxt.config.ts`. VueUse composables are auto-imported. Prefer Nuxt's own
  composables when names overlap, including `useFetch`, `useHead`, and
  `useCookie`.
- Keep the frontend `typecheck` script and `tsconfig.json` working. `pnpm check`
  and `nstack check` run that script after validating the backend, so Vue
  templates and route components must pass `vue-tsc`.
...
- Use `apiClient()` from `frontend/app/utils/api.ts` for frontend calls to
  ...
  in sync using local Encore metadata. Use it inside `useAsyncData()` for page
  loads and call it directly from event handlers for mutations.
...
- `paseo.json` runs `pnpm worktree`, which keeps Paseo sessions on the nstack
  dev path and avoids bypassing client sync, setup checks, and port handling.
- Keep backend resource declarations in Encore source. Let `nstack deploy`
  reconcile Dokploy from Encore metadata.
- Generated public services join Dokploy's `dokploy-network` with Traefik
  labels so `/` routes to Nuxt, `/api` routes to Encore, and public object
  paths route to the object proxy.
- `nstack status` checks Dokploy records, generated Traefik routing labels, and
  configured public verify endpoints. A plain Traefik `404 page not found`
  means the live router is missing or stale; run `nstack deploy`.
- Provider-backed Git push deploys use infrastructure credentials prepared by
  `nstack deploy`. After declaring a new database, cache, or object storage
  bucket, run `nstack deploy` before relying on push deployment. Missing
  infrastructure credentials stop the push deployment. Push deploys do not
  delete removed resources.
- Provider-backed `nstack deploy` updates Compose and the private Dokploy
  environment before pushing, then waits for the push webhook deployment. On
  source push authentication failures, nstack prints the provider auth settings
  URL and opens it unless `--print`, `--no-browser`, or `--json` is used.
...
- Use `nstack env set`, `nstack env pull`, and `nstack env push` for runtime
  secrets. Local target, state, and secret files use private permissions. Never
